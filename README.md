# Skyfall Dashboard

⚠️ **ВАЖНО ПЕРЕД НАЧАЛОМ РАБОТЫ**:

Этот проект требует создания собственных ключей аутентификации. **НЕ используйте чужие ключи!**

1. Создайте аккаунт на https://dashboard.clerk.com/
2. Создайте новое приложение
3. Скопируйте свои ключи
4. Создайте файл `.env.local` в корне проекта
5. Добавьте свои ключи в `.env.local`

Подробная инструкция в разделе "Настройка переменных окружения" ниже.

---

## Предварительные требования

- Node.js
- npm (устанавливается с Node.js)
- PostgreSQL (версия 14 или выше) - [Скачать](https://www.postgresql.org/download/)

## 🚀 Быстрый старт

### 1. Клонирование репозитория

```bash
git clone https://github.com/Rom2555/skyfall-dashboard.git
cd skyfall-dashboard
```

### 2. Установка зависимостей

```bash
npm install
```

### 3. Настройка переменных окружения (ОБЯЗАТЕЛЬНО!)

⚠️ **ВАЖНО**: Вы должны создать свой файл `.env.local` с вашими собственными ключами Clerk. Не используйте чужие ключи!

Скопируйте файл `.env.example` в `.env.local`:

```bash
cp .env.example .env.local
```

Откройте `.env.local` и **ОБЯЗАТЕЛЬНО** замените все значения на свои:

```text
# Clerk Authentication - СОЗДАЙТЕ СВОЙ АККАУНТ НА https://dashboard.clerk.com/
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=ВАШ_PUBLISHABLE_KEY
CLERK_SECRET_KEY=ВАШ_SECRET_KEY

# Database - ИЗМЕНИТЕ НА СВОИ ДАННЫЕ
DATABASE_URL=postgresql://ВАШ_USER:ВАШ_PASSWORD@localhost:5432/skyfall

# Facebook App (опционально)
FACEBOOK_APP_ID=ВАШ_FACEBOOK_APP_ID
FACEBOOK_APP_SECRET=ВАШ_FACEBOOK_APP_SECRET

# Trigger.dev (опционально)
TRIGGER_API_KEY=ВАШ_TRIGGER_KEY
TRIGGER_SECRET_KEY=ВАШ_TRIGGER_SECRET

# App
NEXT_PUBLIC_APP_URL=http://localhost:3000
```

📌 **Примечание**: Файл `.env.local` не должен быть в Git и содержит ваши секретные ключи.

### 4. Настройка PostgreSQL

Установите PostgreSQL с официального сайта https://www.postgresql.org/download/ и создайте базу данных:

```sql
CREATE DATABASE skyfall;
```

### 5. Применение миграций базы данных

```bash
# Генерация Prisma Client
npx prisma generate

# Применение миграций
npx prisma migrate dev

# (Опционально) Заполнение базы тестовыми данными
npx prisma db seed
```

### 6. Запуск сервера разработки

```bash
npm run dev
```

Приложение будет доступно по адресу http://localhost:3000

## 🔑 Получение API ключей

### Clerk (Аутентификация)

1. Перейдите на https://dashboard.clerk.com/
2. Создайте новый проект или выберите существующий
3. В разделе "API Keys", скопируйте:
   - Publishable Key → `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`
   - Secret Key → `CLERK_SECRET_KEY`
4. Настройте URL-адреса для входа/регистрации в настройках Clerk

### Facebook Ads API

1. Перейдите на https://developers.facebook.com/
2. Создайте новое приложение
3. В настройках приложения добавьте продукт "Marketing API"
4. Скопируйте:
   - App ID → `FACEBOOK_APP_ID`
   - App Secret → `FACEBOOK_APP_SECRET`
5. Настройте Redirect URI: `http://localhost:3000/api/auth/callback/facebook`

### Trigger.dev (Опционально)

1. Перейдите на https://trigger.dev/
2. Создайте новый проект
3. Скопируйте API ключи в `.env`

## 🛠 Доступные команды

```bash
# Запуск сервера разработки
npm run dev

# Сборка для продакшена
npm run build

# Запуск продакшн-сервера
npm start

# Линтинг кода
npm run lint

# Генерация Prisma Client
npx prisma generate

# Применение миграций
npx prisma migrate dev

# Просмотр базы данных в браузере
npx prisma studio

# Сброс базы данных (осторожно!)
npx prisma migrate reset
```

## 📁 Структура проекта

```
skyfall-dashboard/
├── prisma/
│   ├── schema.prisma          # Схема базы данных
│   ├── migrations/            # Миграции базы данных
│   └── seed.ts                # Начальные данные
├── src/
│   ├── app/                   # Next.js App Router
│   ├── components/            # React компоненты
│   └── lib/                   # Утилиты и хелперы
├── public/                    # Статические файлы
├── .env.example               # Пример переменных окружения
├── package.json               # Зависимости проекта
└── README.md                  # Этот файл
```

## 🗄 Модели базы данных

Проект использует следующие основные модели:

- `users` - Пользователи системы
- `tenants` - Организации/клиенты
- `campaigns` - Рекламные кампании
- `metrics_daily` - Ежедневные метрики кампаний
- `ad_accounts` - Рекламные аккаунты
- `ai_recommendations` - AI рекомендации
- `predictions` - Прогнозы эффективности

## 🛠 Troubleshooting

### ⚠️ Проблема с аутентификацией / "Некорректный владелец"

Если вы видите ошибку аутентификации или сообщение о некорректном владельце:

**ПРИЧИНА**: Вы используете чужие ключи Clerk или не создали свой `.env.local` файл.

**РЕШЕНИЕ**:
1. Создайте свой аккаунт на https://dashboard.clerk.com/
2. Создайте новое приложение в Clerk
3. Скопируйте свои ключи из раздела "API Keys"
4. Создайте файл `.env.local` в корне проекта
5. Добавьте в него свои ключи:

```text
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=ВАШ_PUBLISHABLE_KEY
CLERK_SECRET_KEY=ВАШ_SECRET_KEY
DATABASE_URL=postgresql://ВАШ_USER:ВАШ_PASSWORD@localhost:5432/skyfall
NEXT_PUBLIC_APP_URL=http://localhost:3000
```

6. Перезапустите сервер: `npm run dev`

### Ошибка подключения к базе данных

Убедитесь, что PostgreSQL запущен и параметры подключения в `.env` верны:

```bash
# Проверка подключения PostgreSQL
psql -U postgres -d skyfall -h localhost
```

### Ошибка Prisma Client

Перегенерируйте Prisma Client:

```bash
npx prisma generate
```

### Проблемы с миграциями

Если миграции не применяются, попробуйте сбросить базу данных:

```bash
npx prisma migrate reset
```

### Ошибки Clerk

Проверьте, что ключи Clerk правильно скопированы в `.env` и приложение настроено с правильными URL-адресами.

## 🚀 Развертывание

### Vercel (рекомендуется)

1. Подключите репозиторий к Vercel
2. Добавьте переменные окружения в настройках проекта
3. Разверните проект

### Другие платформы

Для развертывания на других платформах убедитесь, что:

- Установлены все зависимости (`npm install`)
- Выполнена сборка (`npm run build`)
- Настроена база данных PostgreSQL
- Добавлены все переменные окружения

## 📜 Лицензия

Этот проект распространяется под лицензией MIT.

## 🤝 Вклад

Вклады приветствуются! Пожалуйста, создавайте issue или pull request для любых улучшений.

## 📞 Поддержка

Если у вас возникли вопросы или проблемы, пожалуйста, создавайте issue в репозитории GitHub.
